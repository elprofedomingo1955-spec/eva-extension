...
const DOM_REFS = {
  ...
  downloadSeqBtn: document.getElementById('downloadSeq'),
...
/**
 * Muestra el modal de confirmación con el texto dado
 * @param {string} text - Texto inicial para el modal
 * @param {Function} onConfirm - Función a llamar al confirmar
 */
function showModal(text, onConfirm) {
  DOM_REFS.modalMessage.value = text;
  DOM_REFS.confirmModal.style.display = 'block';
  DOM_REFS.modalMessage.focus();

  // Event listener temporal para confirmar
  const confirmHandler = () => {
    hideModal();
    onConfirm(DOM_REFS.modalMessage.value);
  };
  DOM_REFS.modalConfirm.addEventListener('click', confirmHandler, { once: true });

  // Comportamiento de teclado en modal
  const keyHandler = (e) => {
  ...
  DOM_REFS.modalMessage.addEventListener('keydown', keyHandler);
  DOM_REFS.modalCancel.addEventListener('click', () => {
    hideModal();
    DOM_REFS.modalMessage.removeEventListener('keydown', keyHandler);
  }, { once: true });
}
 ...
 */
function hideModal() {
  DOM_REFS.confirmModal.style.display = 'none';
}
...
function setupEventListeners(handlers) {
  ...
  // Captura del primer carácter y permitir Enter como salto de línea
  let firstCharCaptured = false;
  DOM_REFS.message.addEventListener('keydown', (e) => {
    // CORREGIDO: Permitir Enter normal como salto de línea
    // Ya no prevenir el comportamiento por defecto de Enter
  ...
  DOM_REFS.downloadSeqBtn.addEventListener('click', handlers.downloadSequential);
  ...
  if (DOM_REFS.observerSelector) {
    DOM_REFS.observerSelector.addEventListener('change', handlers.handleObserverSelectorChange);
  }
}
